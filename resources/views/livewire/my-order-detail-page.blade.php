      ...
      <div class="md:w-3/4">
        <div class="bg-white overflow-x-auto rounded-lg shadow-md p-6 mb-4">
          <table class="w-full table-auto">
            <thead>
                <tr class="border-b">
                    <th class="text-left font-semibold px-4 py-2 w-2/5">Product</th>
                    <th class="text-left font-semibold px-4 py-2 w-1/5">Price</th>
                    <th class="text-left font-semibold px-4 py-2 w-1/5">Quantity</th>
                    <th class="text-left font-semibold px-4 py-2 w-1/5">Total</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($order_items as $item)
                @php
                    $tax = $item->order->grand_total * 0.10;
                    $total = $item->order->grand_total + $tax;
                @endphp
                    <tr  wire:key={{$item->id}}>
                        <td class="py-4 px-4">
                            <div class="flex items-center">
                                <img class="h-16 w-16 object-cover rounded-lg mr-4" src="{{url('storage', $item->product->images[0])}}" alt="{{$item->product->name}}">
                                <span class="font-semibold text-sm md:text-base">{{$item->product->name}}</span>
                            </div>
                        </td>
                        <td class="py-4 px-4 text-sm md:text-base">
                            Rp.{{ number_format($item->unit_amount, 0, ',', '.') }}
                        </td>
                        <td class="py-4 px-4 text-center text-sm md:text-base">
